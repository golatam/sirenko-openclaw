...
Плагин (`services/gateway/work-agent/`) — модульная архитектура, 31 тул. `index.ts` — оркестратор (конфиг, MCP-клиенты, `PluginContext` DI-контейнер), делегирует регистрацию тулов в доменные модули: `gmail.ts`, `calendar.ts`, `drive.ts`, `sheets.ts`, `docs.ts`, `analytics.ts` (GA4 + Tally + Amplitude), `slack.ts`, `ops.ts` (cross-domain: search, reports, health, backup, usage). Shared: `types.ts` (PluginContext interface), `clients.ts` (HTTP-клиенты: Tavily, Tally, Telegram search, Slack API), `health.ts` (monitoring + periodic checks), `utils.ts` (fetchWithTimeout, confirmationId, `jlog` structured logger, `reqId` trace ID). MCP-клиент (`mcp-client.ts`) — класс `McpClient` с per-instance session state и поддержкой auth providers (InternalAuthProvider для сайдкаров, OAuthBearerProvider для Amplitude). Инстансы: `googleMcp` (Google) и `amplitudeMcp` (Amplitude — official MCP server). Telegram-данные из PostgreSQL через HTTP. Tally.so данные — прямой REST API из плагина (без MCP).

Логирование: все сервисы используют structured JSON logging (JSON lines на stderr). Формат: `{"ts", "level", "service", "msg", "data"}`. Railway Logs автоматически парсят JSON.
...
- **Merge (union by ID)**: `cron/jobs.json` — определения jobs из `cron-seed.json`, runtime state (`nextRunAtMs`, `lastRunAtMs` и т.д.) сохраняется из volume. Runtime-created jobs (не в seed) тоже переживают деплой. Это критично для `runMissedJobs()` — без сохранённого state пропущенные jobs не обнаруживаются.
...
Сборка и линтер не настроены. Плагин загружается OpenClaw динамически. CI: `.github/workflows/ci.yml` — Docker build всех 4 сервисов + Trivy security scan.
