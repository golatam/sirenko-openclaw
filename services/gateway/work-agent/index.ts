...
async function probeAllSidecars(): Promise<HealthResult> {
...
function formatHealthMessage(result: HealthResult, kind: AlertKind): string {
...
async function runPeriodicHealthCheck(): Promise<void> {
  const result = await probeAllSidecars();
  const prev = loadLastHealthStatus();
  saveHealthStatus(result.status);

  if (prev === null) {
    // First run — only notify if not ok
    if (result.status !== "ok") {
      await sendHealthNotification(result, "alert");
    }
  } else if (prev !== "ok" && result.status === "ok") {
    await sendHealthNotification(result, "recovery");
  } else if (prev === "ok" && result.status !== "ok") {
    await sendHealthNotification(result, "alert");
  }
  // same status → silent
...
const WorkAgentPlugin = {
  ...
  register(api: OpenClawPluginApi) {
    ...
    // -----------------------------------------------------------------------
    // Google Sheets (via google-mcp-sidecar)
    // -----------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
    // Google Docs (via google-mcp-sidecar)
    // -----------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
    // Post-deploy health check (3 min after startup) + periodic (every 30 min)
    // -----------------------------------------------------------------------
    ...
    setTimeout(async () => {
      try {
        const result = await probeAllSidecars();
        saveHealthStatus(result.status);
        await sendHealthNotification(result, "post-deploy");
      ...
      // Start periodic checks after the post-deploy check
      setInterval(async () => {
        try {
