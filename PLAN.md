...
OpenClaw v2026.3.2 имеет встроенный media pipeline для аудио (`tools.media.audio`). Достаточно конфига + API key.
...
- [x] Абстракция `adapter.ts` над OpenClaw Plugin SDK (защита от обновлений) (2026-03-05)
...
### 8b — Google Analytics Plugin Tools (Done, 2026-03-05)
- [x] GA4 тулы уже в google-mcp-sidecar: `analytics_list_properties`, `analytics_run_report`
- [x] Plugin обёртки: `work_analytics_properties`, `work_analytics_report` — в `index.ts`
- [x] OAuth credentials — используются существующие scopes google-mcp-sidecar (analytics.readonly добавлен в Phase 10)
...
## Phase 11 — MVP Test Monitoring (Tally + GA4 + Google Ads)

Мониторинг MVP-тестов: формы (Tally), трафик и поведение (GA4), рекламные кампании (Google Ads через GA4 linked reports).

### 11a — Tally.so (Done, 2026-03-09)
- [x] `work_tally_forms` — список форм с количеством ответов и статусом
- [x] `work_tally_submissions` — ответы на форму (фильтры: дата, статус, пагинация)
- [x] REST API клиент (`tallyGet()`): `https://api.tally.so`, Bearer token, прямой fetch из плагина
- [x] Config pipeline: `TALLY_API_KEY` → `openclaw.json` → `configSchema` → `adapter.ts` → `index.ts`

### 11b — Google Ads через GA4 (Done, 2026-03-09)
- [x] Google Ads данные доступны через существующий `work_analytics_report` — GA4 property привязана к Ads
...
- Не требует отдельного OAuth scope, Developer Token или нового кода — всё через GA4 Data API

## Phase 12 — Архитектурные улучшения (Codex Audit v2, 2026-03-09)

Источник: анализ Codex (gpt-5.3-codex, xhigh reasoning) от 2026-03-09. Приоритизировано по ROI (impact / effort).

### 12a — Quick Wins (низкая сложность, высокий ROI)

**Качество кода:**
- [x] Вынести hardcoded бизнес-константы в env/config с валидацией (`SLACK_ALERT_CHANNEL`, `BACKUP_ACCOUNT`, `BACKUP_RETENTION_DAYS` → `WorkAgentConfig` + `configSchema`) (2026-03-09)
- [ ] Единый source of truth для plugin configSchema — генерация manifest из TS/Zod (drift между `index.ts:363` и `openclaw.plugin.json:5`)

**Надёжность:**
- [x] Единый guard для MCP-клиентов — `requireGoogleMcp()` / `requireAmplitudeMcp()` вместо `!` assertion, graceful error через try/catch (2026-03-09)
- [ ] Cron merge "union by id": seed обновляет known jobs, runtime-задачи сохраняются (сейчас runtime jobs теряются при деплое, `entrypoint.sh:40`)

**Безопасность:**
- [x] `SIDECAR_AUTH_TOKEN` прокинут во все сервисы в docker-compose.yml (2026-03-09)
- [x] Slack: `dmPolicy: "pairing"`, `allowFrom: ["slack:U0AH7S5AG91"]`, `requireMention: true` в группах (2026-03-09)

**DevOps:**
- [x] `.dockerignore` во всех 4 сервисах + telegram-sidecar: `COPY . /app` → selective `COPY main.py schema.sql /app/` (2026-03-09)

### 12b — Средний приоритет (средняя сложность, высокий ROI)

**Качество кода:**
- [x] Разбить `index.ts` (2103 строки) на модули по доменам: `gmail.ts`, `calendar.ts`, `drive.ts`, `sheets.ts`, `docs.ts`, `analytics.ts`, `slack.ts`, `ops.ts` + `types.ts`, `clients.ts`, `health.ts` + PluginContext DI (2026-03-09)
- [ ] Формализовать контракты между сервисами (JSON Schema / OpenAPI) + runtime validation на границах

**Производительность:**
- [ ] Параллелить per-account запросы в google-mcp-sidecar (`asyncio.gather` + semaphore вместо последовательного обхода в `server.py:195,358,387`)
- [ ] Batch insert / COPY для ingest в telegram-sidecar и whatsapp-sidecar (сейчас 1 INSERT на сообщение, `main.py:225`, `main.js:340`)

**Надёжность:**
- [ ] Единая migration-система (Alembic или raw SQL migrations) вместо дублирования схемы между сайдкарами. WA-сайдкар `ON CONFLICT` по dedup-ключу есть (`main.js:100`), но unique index не создаётся (`main.js:55`)
- [ ] Distributed lock (Postgres advisory lock) для health/backup циклов — подготовка к горизонтальному масштабированию (`index.ts:2002,2096`)

**Безопасность:**
- [ ] Non-root user + read-only rootfs + drop capabilities во всех Dockerfiles (сейчас контейнеры от root)
- [ ] WhatsApp `/qr`: перевести auth на header (сейчас токен в query string, `main.js:404`) + локальный QR без внешнего CDN JS (`main.js:419`)

**DevOps:**
- [ ] CI-пайплайн: lint + typecheck + docker build + Trivy security scan (`.github/workflows/` отсутствует)
- [ ] Structured JSON logging + trace/request ID через gateway→sidecar (сейчас `print`/`console.error` без correlation, `main.py:431`, `server.py:57`)

### 12c — Долгосрочные (высокая сложность, стратегический ROI)

**Производительность:**
- [ ] Партиционирование `messages` по `ts` (месяц) + generated `tsvector` + индексы под `sender`/`chat_type` фильтры (`schema.sql:12`)
- [ ] Async очередь для транскрипции голосовых (Redis queue + workers) — убрать backpressure при rate-limit STT API (`main.py:299`, `main.js:326`)

**Observability:**
- [ ] Prometheus-метрики + SLO-алерты: ingest lag, account disconnected, backup age, 5xx rate (сейчас только health endpoints)

### Приоритизация (roadmap)

**Спринт 1 (quick wins, ~3-5 дней):**
1. `.dockerignore` + оптимизация COPY
2. Guard для MCP-клиентов (graceful degradation)
3. Hardcoded константы → config
4. Sidecar auth обязательный в docker-compose
5. Slack allowlist

**Спринт 2 (средние задачи — Done, 2026-03-09):**
1. [x] Разбиение `index.ts` на модули (Phase 12b)
2. [x] Schema sync: WA-сайдкар синхронизирован с каноническим schema.sql (полный GIN-индекс, dedup, UNIQUE)
3. [x] CI-пайплайн: `.github/workflows/ci.yml` (Docker build + Trivy scan)
4. [x] Non-root Docker: `USER appuser` в 3 сайдкарах
5. [x] Structured JSON logging: все сервисы → JSON lines на stderr
6. [x] configSchema sync: `index.ts` ↔ `openclaw.plugin.json` (14 свойств)
7. [x] Cron merge union by ID: runtime jobs переживают деплой
